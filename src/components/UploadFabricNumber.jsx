      ...
      complete: async (results) => {
        ...
        try {
          const formattedData = results.data.map((row) => {
            ...
            const fabrics = [];
            for (let i = 1; i <= 3; i++) {
              const fabricNo = row[`Fabric ${i}`];
              const fabricName = row[`Fabric ${i} Name`];
              const fabricImg = row[`Fabric ${i} Image`];
              if (fabricNo || fabricName || fabricImg) {
                fabrics.push({
                  fabric_no: fabricNo || null,
                  fabric_name: fabricName || "",
                  fabric_image: fabricImg || "",
                });
              }
            }
          ...
          setResult(null);
          console.log("Failed to upsert style fabric number details error :: ", err);
        } finally {
          setUploading(false);
